...
An AI agent that searches multiple literature databases for a research question, screens abstracts and full text, and summarizes key findings, themes, and research gaps — verifying that every citation was actually retrieved (not just found in search) and grading each paper's evidence strength (systematic review, RCT, observational study, etc.).
...
- **Abstract-only disclosure** — retrieval prefers full-text chunks over abstract chunks for the same paper when both exist, and any citation still grounded only in an abstract is labeled `abstract only` — abstracts routinely omit the methodology and effect-size detail that matters for a real claim.
...
The frontend is a static Vite build (fits any static host — Vercel, Netlify). The backend needs a real long-running process, not a serverless function — requests stream over SSE for 60-150+ seconds and hold a loaded embedding model in memory, which doesn't fit a short-execution serverless model (Railway/Render/Fly.io all work).
